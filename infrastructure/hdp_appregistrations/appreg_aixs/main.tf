locals {
  app_name = "appreg-aixs-s2s-hdp-${var.app_env}-${var.region_code}"
}
...
resource "azuread_application_password" "appreg_aixs_application_password" {
  ...
  display_name          = "appreg-aixs-s2s-hdp-client-secret-${var.app_env}-${var.region_code}"
}

resource "azurerm_key_vault_secret" "appreg_aixs-s2s-hdp_client_secret_key_vault" {
  name         = "appreg-aixs-s2s-hdp-client-secret-${var.app_env}-${var.region_code}"
...
resource "azurerm_key_vault_secret" "appreg_aixs-s2s-hdp_client_id_key_vault" {
  name         = "appreg-aixs-s2s-hdp-client-id-${var.app_env}-${var.region_code}"
